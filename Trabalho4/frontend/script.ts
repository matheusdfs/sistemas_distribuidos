...
btnGetItinerarios.addEventListener('click', async () => {
  try {
    ...
    const tabela = document.getElementById('itinerarios') as HTMLTableElement;
    tabela.innerHTML = `
      <tr>
        <th>Código</th>
        <th>Origem</th>
...
btn.addEventListener('click', async () => {
  try {
    const codigoItinerario = (document.getElementById('codigoItinerario') as HTMLInputElement).value;
...
btnCancelar.addEventListener('click', async () => {
  try {
    const codigoReserva = (document.getElementById('codigoReserva') as HTMLInputElement).value;
